...
export default function TopNav() {
  ...
  return (
    <nav style={{
      ...
      background: 'rgba(10,10,15,0.65)', backdropFilter: 'blur(28px) saturate(1.3)', WebkitBackdropFilter: 'blur(28px) saturate(1.3)',
      borderBottom: '1px solid rgba(255,255,255,0.1)',
      boxShadow: '0 4px 30px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.05)'
    }}>
      <div className="container bar-inner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
        {/* Left: Logo or back */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {canGoBack(location.pathname) ? (
            ...
            <span style={{ fontSize: 20, fontFamily: 'var(--font-display)', fontWeight: 800, flexShrink: 0 }}>
          ...
          {/* Desktop nav links */}
          <div className="topnav-links" style={{ marginLeft: 18 }}>
            {navLinks.map(l => (
              <button key={l.path} onClick={() => navigate(l.path)} className={`topnav-link ${location.pathname === l.path ? 'active' : ''}`}>
                {l.label}
              </button>
            ))}
          </div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showNotif && (
                  <div style={{
                    ...
                    background: 'rgba(22,22,31,0.85)', backdropFilter: 'blur(24px) saturate(1.3)', WebkitBackdropFilter: 'blur(24px) saturate(1.3)',
                    border: '1px solid rgba(255,255,255,0.1)',
                    ...
                    {notifications.length === 0 ? (
                      ...
                      <div key={i} onClick={() => { setShowNotif(false); if (n.orderId) navigate(`/order-tracking/${n.orderId}`); }} style={{
                        ...
                        cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)', transition: 'background 0.15s ease'
                      }}>{n.message}</div>
                    ))}
                  </div>
                )}
